...
from .database import Table, Feature, FeatureCollection
...
def create_layer(features: Union[List[QgsFeature], QgsFeatureIterator],
    ...
    rename : dict, optional
        rename fields, field names as keys, new names as values
    name : str, optional
    ...
    for field in fields:
        pr.addAttributes([QgsField(rename.get(field, field), QVariant.String)])
    layer.updateFields()
    if target_epsg:
        tr = QgsCoordinateTransform(
...
def intersect(input_features: Union[List[Feature], FeatureCollection],
              overlay: Union[QgsVectorLayer, List[Feature], FeatureCollection],
              input_fields={'id': 'id'}, output_fields={}, epsg: int = 4326,
    ...
    rename_input = dict([(k, v) for k, v in input_fields.items() if k != v])
    input_layer = create_layer(input_features, 'Point',
                               fields=input_fields.keys(),
                               rename=rename_input, name='input', epsg=epsg)
    if not isinstance(overlay, QgsVectorLayer):
        rename_output = dict([(k, v) for k, v in output_fields.items() if k != v])
        overlay = create_layer(overlay, 'Polygon', rename=rename_output,
                               name='overlay', fields=output_fields.keys(),
    ...
    ret_fields = list(input_fields.values()) + list(output_fields.values())
